...
The line-1 right cluster sheds under width pressure in priority order: the description truncates first, then `<share>%` is dropped, then the `<N>K` token count — while `<model>` and the front `<elapsed>` always render. Independently of width, `<share>%` is omitted when the session denominator is 0. The `<N>K` token count is right-justified to a 5-col field and the model to a (minimum) 6-col field so the `·` separators line up across rows. `<model>` is the model bucket name (`opus`/`sonnet`/…); a subagent running the **1M-token context variant** of its model carries the literal `[1m]` marker in its raw model id and renders the label as `opus[1m]`, widening the field past 6 for that row (the colour bucket is unchanged). The **t/m rate** and **↑output** fields are no longer rendered in any form (removed).
