...
### 5.8 Measured after implementation — the render engine, not scanning, dominates per-render cost *(finding R1)*
Once the scan optimizations were in, the realistic benchmark (`ops/bench.py --transcript-lines N`, this branch vs `main`) plus in-process phase profiling produced an important, humbling result that the original audit missed:
- **Bare `python3` startup is only ~13 ms** here — not the dominant cost.
- The **warm incremental scan is ~0.5 ms**; single-pass + incremental make `main` ~3% slower at 4 000 transcript lines and **~23% (~37 ms/render) slower at 12 000 lines** — so the scan win is real and **grows with session length**, but is small for short transcripts.
- The **dominant per-render cost is the pure-Python gradient/ANSI render engine**: `build_narrow` ≈ 0.04 ms, `build_medium` ≈ 37 ms, `build_wide` ≈ 60–100 ms — independent of transcript size *and* terminal width, and unchanged when token-log I/O is stubbed. The hot path is per-cell RGB interpolation (`grad_at`/`spark_rgb`, hundreds of calls per render). **(measured — `ops/bench.py`, phase profiling)**

This reorders the performance priorities: the scan/I/O fixes were correct and help long sessions, but **the highest-value remaining latency target is the render engine (R1)** — memoize/precompute gradients or cache rendered segments — and it strengthens the case for the compiled core (Phase 3), which would help the render math most.
...
| R1 | CPU (render) | **Medium-High** | Pure-Python gradient/ANSI engine dominates per-render cost (build_wide ~60–100ms vs scan ~0.5ms) — measured, missed by the first draft | `ops/bench.py` + phase profiling; `grad_at`/`spark_rgb` | Memoize/precompute gradients, cache rendered segments; compiled core (Phase 3) |
