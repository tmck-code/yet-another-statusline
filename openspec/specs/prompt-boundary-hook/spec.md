# prompt-boundary-hook Specification

## Purpose

Define how the YAS plugin ships a `UserPromptSubmit` hook that records prompt-submit timestamps in a shared, atomically-written state file, and how the statusline consumes that file to establish a per-session turn boundary for cohort scoping.
