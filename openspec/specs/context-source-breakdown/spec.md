# context-source-breakdown Specification

## Purpose

Record the constraint and deferral around showing a by-source context-window breakdown (system prompt vs tool definitions vs plugins vs skills vs messages). Investigation shows the data required for a literal system-vs-plugins-vs-skills breakdown is not available to the statusline today, so this capability documents the gap and the feasible alternatives rather than implementing a breakdown that cannot be derived.
