...
def test_wide_target_returns_full_form() -> None:
    git = _git()
    pwd = '~/proj'
    full = _r.path_git(pwd, git)
    result = _r.fit_path(pwd, git, _visible_width(full) + 10)
    assert result == full
...
def test_drops_commit_first() -> None:
    git = _git()
    pwd = '~/proj'
    no_commit = _r.path_git(pwd, git, show_commit=False)
...
def test_drops_dirty_after_commit() -> None:
    git = _git()
    pwd = '~/proj'
    no_dirty = _r.path_git(pwd, git, show_commit=False, show_dirty=False)
...
def test_branch_only_omits_path_whole() -> None:
    ...
    compact = _r.path_git_compact(pwd, git)
...
def test_floor_never_overflows_across_small_widths() -> None:
    ...
    floor_w = _visible_width(_r.path_glyph_only())
    assert floor_w == 1
...
def test_compact_only_skips_full_path_git_stages() -> None:
    ...
    result = _r.fit_path(pwd, git, _visible_width(full) + 50, compact_only=True)
    stripped = strip_ansi(result)
    assert result == _r.path_git_compact(pwd, git)
    assert git.commit not in stripped
    assert ELLIPSIS not in stripped
...
def test_compact_only_ladder_terminates_at_branch_then_glyph() -> None:
    ...
    result = _r.fit_path(pwd, git, _visible_width(branch_only), compact_only=True)
    assert result == branch_only
    assert pwd not in strip_ansi(result)
    assert ELLIPSIS not in strip_ansi(result)

    # Glyph-only floor under compact_only at a tiny width.
    floor = _r.fit_path(pwd, git, 1, compact_only=True)
    assert floor == _r.path_glyph_only()
